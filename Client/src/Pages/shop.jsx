...
function shop({ shopData, addToCart }) {
  ...
  const {
    data: { productsData, categoriesData },
  } = shopData;

  let dumyData = categoriesData.find((item) => item.title === data);
  // set Products on Button Click
  const handleCategory = (value, title) => {
    setHeading(title);
  ...
  useEffect(() => {
    ...
    if (data === undefined) {
      handleCategory(categoriesData[0]._id, categoriesData[0].title);
    } else {
      handleCategory(dumyData._id, dumyData.title);
    }
    setFlag(true);
  }, [products]);
      ...
      <div className="container mx-auto my-4 px-4">
        <div className="flex justify-center items-center gap-6">
          {shopData?.data?.categoriesData?.map((item) => (
            <Button
              key={item._id}
              onClick={() => handleCategory(item._id, item.title)}
            >
              {item.title}
            </Button>
          ))}
              ...
              <Card key={index} className="mt-6 ">
                ...
                <CardFooter className="pt-0 mt-auto flex items-center justify-between">
                  <Button
                    onClick={() => {
                      addToCart(item);
                      setHideCartBtn(item._id);
                    }}
                    className="flex justify-start items-center gap-1"
                    disabled={hideCartBtn === item._id ? true : false}
                  >
                    {`${hideCartBtn === item._id ? "Added" : "Add to Cart"}`}{" "}
                    {hideCartBtn === item._id ? (
